...
OUTPUT_PNG = "atomic_card_table_v6.png"
...
for i, row in mart.iterrows():
    ...
    # 薬剤名（⑦カプレット注記マーク）
    name_text = row["product"] + ("　※カプレット" if row["is_caplet"] else "")
    ax.text(COL_NAME_X, current_y, name_text, fontsize=10.0 if not row["is_caplet"] else 9.0,
            fontweight="bold", ha="left", va="center")

    # 対象成分（塩酸塩などの文字列をカット）
    if row["ingredients"]:
        clean_ingr = re.sub(r'(塩酸塩|リン酸塩|硫酸塩|臭化水素酸塩|マレイン酸塩|酒石酸塩|フマル酸塩)', '', row["ingredients"])
        ax.text(COL_INGR_X, current_y, clean_ingr, fontsize=7.5, ha="left", va="center", color="#666666")
    else:
...
trap_start_y = current_y

for brand, left_text, right_text in trap_list:
    ax.text(TRAP_BRAND_X, current_y, f"・{brand}", fontsize=9.0, ha="left", va="center", color="#444444")
    ax.text(TRAP_LEFT_X, current_y, left_text, fontsize=9.0, ha="left", va="center", color="#444444")
    separator = "⇔" if "該当" in right_text else ""
    ax.text(TRAP_ARROW_X, current_y, separator, fontsize=9.0, ha="center", va="center", color="#444444")
    text_color = "#d32f2f" if "該当" in right_text else "#444444"
    ax.text(TRAP_RIGHT_X, current_y, right_text, fontsize=9.0, ha="left", va="center", color=text_color)
    current_y -= 2.2

# ⑦ カプレット注記の脚注（該当行が多いため一覧補足として下部にも一言）
current_y -= 1.5
ax.text(COL_NAME_X, current_y,
        "※カプレット表記の品目（ベンザブロック◯◯／末尾「錠」なし）は「◯◯錠」と1日成分量は同一ですが、服用粒数が異なります。",
        fontsize=7.5, ha="left", va="center", color="#888888")

# --- 右下：QRコード×2（④判定アプリ／⑤意見フォーム）とURL併記 ---
qr_size = 10.0
qr_y_center = trap_start_y - 6.5
...
ax.text(qr1_x, qr_y_center - qr_size/2 - 1.3, "判定アプリ\n(GEMINI Pro 3.1 試作)",
        fontsize=8.0, ha="center", va="top", color="#555555", fontweight="bold")
# ⑤ URL併記
ax.text(qr1_x, qr_y_center - qr_size/2 - 4.2, APP_URL,
        fontsize=5.0, ha="center", va="top", color="#999999")
...
print("==========================================")
print("v6（0814PM修正案 反映版） 早見表生成完了")
