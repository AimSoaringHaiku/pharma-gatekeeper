...
for color, shape, tag, action, ingr in nursing_rows:
    marker(ax, nursing_x[0] - 2.0, y - 0.3, color, shape, 0.7)
    ax.text(nursing_x[0], y, tag, fontsize=8.0, fontweight="bold", ha="left", va="top")
    ax.text(nursing_x[1], y, action, fontsize=7.0, ha="left", va="top", color=GRAY)
    ax.text(nursing_x[2], y, ingr, fontsize=6.8, ha="left", va="top", color=GRAY)
    y -= 4.2
...
wrapped = textwrap.wrap(disclaimer_text, width=62)
for line in wrapped:
    ax.text(2.0, y, line, fontsize=7.5, ha="left", va="center", color=GRAY)
    y -= 2.0
