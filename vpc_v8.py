...
OUTPUT_PNG = "atomic_card_table_v8.png"
...
# --- タイトル ---
current_y = LOGICAL_H - 2.2
ax.text(LOGICAL_W / 2, current_y, "薬剤別 包装区分 早見表", fontsize=18, fontweight="bold", ha="center", va="center")

# ⓪ 作成日（右上）
today_str = datetime.date.today().strftime("%Y/%m/%d")
ax.text(LOGICAL_W, current_y + 1.8, f"作成日: {today_str}", fontsize=8, ha="right", va="center", color="#888888")

# ③ 試作品・LLM作成・検証済みの旨（「必ず」を削除、タイトル直下の余白を圧縮）
current_y -= 2.6
ax.text(LOGICAL_W / 2, current_y,
        "※本表はAIを用いて作成した試作品です。一次資料（添付文書・KEGG等）との照合検証を重ねていますが、"
        "実使用前に最新の公式情報をご確認ください。",
        fontsize=6.5, ha="center", va="center", color="#888888")
...
current_y -= 1.4
ax.text(COL_INGR_X, current_y, TARGET_INGREDIENTS_LEGEND, fontsize=5.0, ha="left", va="center", color="#999999")

# 非対象成分（紛らわしいが対象外）の凡例 ※追加
current_y -= 1.3
ax.text(COL_INGR_X, current_y,
        "非対象成分（紛らわしいが対象外）: 生薬マオウ／無水カフェイン（アンナカ等）／"
        "プロメタジン等（ジフェンヒドラミン以外の抗ヒス）／アリルイソプロピルアセチル尿素",
        fontsize=5.0, ha="left", va="center", color="#999999")
...
ax.text(COL_NAME_X, current_y,
        ...
        fontsize=6.0, ha="left", va="center", color="#555555")
...
# ※v8：判定注意・参考／トラップ欄／QRブロックを含めた「本当に必要な下部の高さ」を実測で積み上げて算出
#    （旧: 34.0固定はトラップ欄とQRの高さを一切見込んでおらず、QRがはみ出て切れる原因だった）
bottom_reserved_space = (ref_rows * 3.0) + 46.0
...
# ⑦ カプレット注記の脚注（要望どおりメインテーブルの直下＝全体の中段に移動）
current_y -= 1.6
ax.text(COL_NAME_X, current_y,
        "※＝カプレット表記の品目（ベンザブロック◯◯／末尾「錠」なし）。「◯◯錠」と1日成分量は同一ですが服用粒数が異なります。",
        fontsize=6.5, ha="left", va="center", color="#888888")
...
if ref_rows > 0:
    ...
    ax.text(COL_NAME_X, current_y, "判定注意・参考", fontsize=11, fontweight="bold", ha="left", va="center")
    ax.text(30.0, current_y, "指定濫用 判定", fontsize=9, fontweight="bold", ha="center", va="center")
    ax.text(45.0, current_y, "同ブランド内比較・注釈", fontsize=9, fontweight="bold", ha="left", va="center")
    ...
    for j, (_, row) in enumerate(reference.iterrows()):
        if j % 2 == 0:
            ax.add_patch(patches.Rectangle((0, current_y - 1.3), LOGICAL_W, 2.6, facecolor="#fafafa", edgecolor="none"))

        ax.text(COL_NAME_X, current_y, row["product"], fontsize=8.5, fontweight="bold", ha="left", va="center")
        ax.text(30.0, current_y, row["status_text"], fontsize=8.5, fontweight="bold", ha="center", va="center", color=row["status_color"])
        ax.text(45.0, current_y, row["note"], fontsize=7.0, ha="left", va="center")
        current_y -= 3.0
...
print("==========================================")
print("v8（QR切れ修正・余白圧縮・非対象成分凡例追加版） 早見表生成完了")
